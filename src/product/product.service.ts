...
export class ProductService {
    ...
    async createProduct(dto: CreateProductDto): Promise<{ message: string, product: Product }> {
        ...
        const product = this.productRepository.create({ ...dto, category: category })
    ...
    async updateProduct(id: string, dto: UpdateProductDto): Promise<{ message: string, product: Product }> {
        try {
            const existsProduct = await this.productRepository.findOne({ where: { id } })
            if (!existsProduct) {
                throw new NotFoundException("product not found");
            }
            await this.productRepository.update(id, dto)
            const editProduct = await this.productRepository.findOne({ where: { id } })
            if (!editProduct) {
