...
export class WishlistService {
    ...
    async createWishlist(dto: CreateWishlistDto): Promise<{ message: string, wishlist: Wishlist }> {
        try {
            const existsUser = await this.userRepository.findOne({ where: { id: dto.userId } })
            if (!existsUser) {
                throw new NotFoundException("user not found");
            }
            const existsProduct = await this.productRepository.findOne({ where: { id: dto.productId } })
            if (!existsProduct) {
                throw new NotFoundException("product not found");
            }
            const existsWishlist = await this.wishlistRepository.create({ ...dto, user: existsUser, product: existsProduct })
    ...
    async getOneWishlist(id: string): Promise<{ message: string, wishlist: Wishlist }> {
        try {
            const existsWishlist = await this.wishlistRepository.findOne({ where: { id } })
            if (!existsWishlist) {
    ...
    async updateWishlist(id: string, dto: UpdateWishlistDto): Promise<{ message: string, wishlist: Wishlist }> {
        try {
            const existsWishlist = await this.wishlistRepository.findOne({ where: { id } })
            if (!existsWishlist) {
                throw new NotFoundException("wishlist not found");
            ...
            const updateWishlist = await this.wishlistRepository.findOne({ where: { id } })
            if (!updateWishlist) {
                throw new NotFoundException("wishlist not found");
            }
