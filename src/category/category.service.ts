...
export class CategoryService {
    ...
    async createCategory(dto: CreateCategoryDto): Promise<{ message: string, category: Category }> {
        try {
            const existsCategory = await this.categoryRepository.findOne({ where: { categoryName: dto.categoryName } })
            ...
            return { message: "create succesfully category", category: createCategory }
        } catch (error) {
            console.error(error);
    ...
    async getOneCategory(id: string): Promise<{ message: string, category: Category }> {
        try {
            const existsCategory = await this.categoryRepository.findOne({ where: { id } })
            if (!existsCategory) {
